...
function scheduleMXCharging(m3_data, mx_data) {
  ...
  // if the target SoC is greater than the current SoC and charging state isn't Complete, set vehicle for scheduled charging
  if ((target_soc > current_soc) && (mx_data.response.charge_state.charging_state != 'Complete')) {  
    ...
    var minutes = start_time.toString().split(':');
    var total_minutes = ((parseInt(minutes[0]) * 60) + parseInt(minutes[1])).toFixed(0);
...
*/    
    ...
    // send email notification
    var message = 'The Model X is set to charge at ' + estimated_charge_start_time.toString() + '.';
...
function stopChargeVehicle(vin) {
  try {
    var url = 'https://owner-api.teslamotors.com/api/1/vehicles/' + getVehicleId(vin) + '/command/charge_stop';
    var options = {
      "headers": {
        "authorization": "Bearer " + ACCESS_TOKEN
      },
...
function setScheduledCharging(vin, time) {
  try {
    var url = 'https://owner-api.teslamotors.com/api/1/vehicles/' + getVehicleId(vin) + '/command/set_scheduled_charging';
    var data = {
      'enable': 'True',
      'time': time
    };
